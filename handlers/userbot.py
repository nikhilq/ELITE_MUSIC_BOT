...
from config import BOT_USERNAME, BOT_OWNER, X_version, xmartyub_version
from callsmusic.callsmusic import client as XMARTYUB
from handlers.ownerstuff import _check_heroku

# To Block a PM'ed User
@XMARTYUB.on_message(filters.private & filters.command("block", [".", "/"]) & filters.me & ~filters.edited)
async def ubblock(_, message: Message):
  shit_id = message.chat.id
  gonna_block_u = await message.edit_text("`Blocking User...`")
  try:
    await XMARTYUB.block_user(shit_id)
...
@XMARTYUB.on_message(filters.command("unblock", [".", "/"]) & filters.me & ~filters.edited)
async def ubblock(_, message: Message):
  good_bro = int(message.command[1])
  gonna_unblock_u = await message.edit_text("`Unblocking User...`")
  try:
    await XMARTYUB.unblock_user(good_bro)
...
@XMARTYUB.on_message(filters.private & filters.command("chats", [".", "/"]) & filters.me & ~filters.edited)
async def ubgetchats(_, message: Message):
  getting_chats = await message.edit_text("`Checking Your Chats, Hang On...`")
  async for dialog in XMARTYUB.iter_dialogs():
    try:
      total = await XMARTYUB.get_dialogs_count()
...
# Leave From a Chat
@XMARTYUB.on_message(filters.command("kickme", [".", "/"]) & filters.me & ~filters.edited)
async def ubkickme(_, message: Message):
  i_go_away = await message.edit_text("`Leaving This Chat...`")
  try:
    await XMARTYUB.leave_chat(message.chat.id)
...
@XMARTYUB.on_message(filters.private & filters.command("pmlogs", [".", "/"]) & filters.me & ~filters.edited)
@_check_heroku
async def getlogs(client: XMARTYUB, message: Message, app_):
  ...
  if status == "on":
    ...
    try:
      ...
      await XMARTYUB.leave_chat(PM_LOG_CHAT_ID, delete=True)
...
@XMARTYUB.on_message(filters.private)
async def sendpmlol(client: XMARTYUB, message: Message):
  XMARTYUB_ID = int((await XMARTYUB.get_me()).id)
  if message.from_user.id == BOT_OWNER or message.from_user.id == XMARTYUB_ID:
    return
  pmlogchat = PM_LOG_CHAT_ID
  userinfo = await XMARTYUB.get_users(user_ids=message.from_user.id)
